    ...
    def __init__(self, parent=None):
        ...
        for r in range(fieldSize):
            ...
            for c in range(fieldSize):
                ...
                bodylayout.addWidget( self.button[r][c], r, c)
                QtCore.QObject.connect(self.button[r][c], QtCore.SIGNAL("clicked()"), self.revealButtonWrapper )
        mainLayout = QtGui.QGridLayout()
        layouttop = QtGui.QHBoxLayout()
        
        
        layouttop.addWidget(QtGui.QLabel(u'Бомб остолось:'))
        self.bombsfound = 0
        self.bombcount = QtGui.QLabel(str(self.bombsfound) + '/' + str(bombQuantity))
        layouttop.addWidget(self.bombcount)
        self.timer = QtCore.QTimer()
        self.timelabel = QtGui.QLabel(u'0.0 сек')
        self.timelabel.setStyleSheet("background-color: rgb(250, 250, 250); color: rgb(0,0,0)")
        self.newGame()
        ...
        #self.bodylayout().setSizeConstraint(QtGui.QLayout.SetFixedSize)
